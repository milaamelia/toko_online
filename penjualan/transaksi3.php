    ...
    <div class="form-group">
      <div class="col-sm-12">
        ...
        <div class="col-sm-2"><br/>
         <td>
         <button type='submit'  name="add" title="add" class="btn btn-info">
...
<div class="box-body">
  <table id="example1 " class="table table-bordered table-striped keranjang">
    ...
    <tbody>
    <?php
    $sql = $conn->query("SELECT * FROM tb_detail_jual a JOIN tb_jual b USING(jual_id) JOIN tb_barang c USING(kode_barang) WHERE jual_id = '".@$_SESSION['id']."'");
    ...
    <tr>
      ...
      <td><a href="batal.php?hapus=<?php echo $row['id_detail']; ?>" name="hapus" title="Hapus" class="btn btn-danger" onclick=";">
                   X
</a></td>
      </tr>
    </tr>
  ...
  <div class="col-sm-3"></div>
      <tr>
     <td></td>
     <td><a href="simpan.php?jual_id=<?=$_SESSION['id'] ?>" input type="submit" name="simpan" class="btn btn-success">Transaksi</a></td> 
   </tr>
        </div>
      </div>
   </div>
...
<script type="text/javascript">   
  ...
  if(isset($_POST['add'])){
    ...
    $tgl = date('Y-m-d');
    $cek = mysqli_num_rows($conn->query("SELECT * FROM tb_jual WHERE jual_id='$_SESSION[id]'"));
    if($cek < 1){
      $conn->query("INSERT INTO tb_jual(tgl_jual, kode_pelanggan, total) VALUES('$tgl', '$pelanggan', '$total')");
      $_SESSION['id'] = mysqli_insert_id($conn);
      $query=mysqli_query($conn,"insert into tb_detail_jual(jual_id, kode_barang, qty, harga_total)
       values(".$_SESSION['id'].", '$kode','$qty','$harga')");
    }
    else{
      $query=mysqli_query($conn, "insert into tb_detail_jual(jual_id, kode_barang, qty, harga_total)
       values(".$_SESSION['id'].", '$kode','$qty','$harga')");
